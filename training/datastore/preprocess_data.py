...
class PreprocessData:
    ...
    def make_necessary_df(self, limit_manifest, limit_keywords):
        ...
        if 'keywords' in self.pkg_kwd_df.columns:
            ...
            raise KeyError("Keywords is not present")
    ...
    def extract_unique_packages(self):
        ...
        print(data_with_dep_check)
        unique_packages = self.utility_obj.extract_package_manifest_lst(
            data_with_dep_check)
        return unique_packages

    def create_df_and_dictionaries(self):
        """Create all the necessary dataframes and dictionaries."""
        self.unique_packages = self.extract_unique_packages()
    ...
    def create_extended_pkg_tag_map(self):
        ...
        return self.extended_ptm

    def update_pkg_tag_map(self):
        """Update the existing package tag map."""
        extended_ptm = self.create_extended_pkg_tag_map()
        for package_name in self.package_tag_map.keys():
            more_keywords = set()
            for dependency in self.pkg_dep_map[package_name]:
        ...
        return self.package_tag_map, self.vocabulary
